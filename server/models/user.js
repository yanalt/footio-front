...
var UserSchema = new mongoose.Schema({
  email: {
    ...
    validate: {
      isAsync: true,
      validator: validator.isEmail, //"validator:" is from mongoose, validtaor.isEmail is a function from the validator library
  ...
  tokens: [{ //a single user can have multiple tokens since he can log in from a computer and a phone at once
...
UserSchema.methods.generateAuthToken = function () {
  var user = this;
  var access = 'auth';
  var token = jwt.sign({_id: user._id.toHexString(), access}, process.env.JWT_SECRET).toString(); //makes a json of the encrypted form of _id, and then converts to string
...
UserSchema.methods.removeToken = function (token) {
  ...
  return user.update({
    $pull: { //$pull is a mongodb operator that lets you remove items from an array that match certain criteria.
      tokens: {token} //in the tokens array, we remove the items where token equals to our token.
    }
  });
};
